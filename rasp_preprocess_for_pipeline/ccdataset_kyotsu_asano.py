...
def combine_date_and_time_to_unix(date_str, time_str):
    """
    date_str: '250101' の形式（YYMMDD）
    time_str: '11:00:00.000000' の形式（HH:MM:SS.ssssss）
    return: UNIX時間（float秒）
    """
    combined_str = f"{date_str}-{time_str}"  # '250101-11:10:42.619185'
    dt = datetime.strptime(combined_str, "%y%m%d-%H:%M:%S.%f")
    return dt.timestamp()


# CSVファイルのパス
csv_folder = "/Users/asanosho/Desktop/csv/new"
analyzed_directory = "/Users/asanosho/Desktop/csv/analyzed"  # 保存したいディレクトリに変更してください
...
for csv_filename in os.listdir(csv_folder):
    if csv_filename.endswith(".csv"):
        ...
        date=csv_parts[0].split('-')[1]  #例: 250101
        mouse_name = csv_parts[1]
        day_phase = csv_parts[1] # 例: day00-phase2-session00
        timestamp = csv_parts[1].replace(".csv", "") 
        phasesession = "phase1"
        cond_iti = "random-iti"
        ...
        if cond_iti == "random-iti":
            if "phase1" in phasesession:
                # テンプレートファイル名
                template_filename = "/Users/asanosho/Desktop/csv/daytime_prj-00_3CSRTT_cond-random-iti_mouse-name_day00-phase1-session00___cc_data.h5"
            elif "phase2" in phasesession:
                # テンプレートファイル名
                template_filename = "/Users/asanosho/Desktop/csv/daytime_prj-00_3CSRTT_cond-random-iti_mouse-name_day00-phase2-session00___cc_data.h5"
        #elif cond_iti == "fixed_iti":
            #if "phase1" in phasesession:
                #template_filename = "/Users/asanosho/Desktop/csv/daytime_prj-00_3CSRTT_cond-fixed-iti_mouse-name_day00-phase1-session00___cc_data.h5"
            #elif "phase2" in phasesession:
                # テンプレートファイル名
                #template_filename = "/Users/asanosho/Desktop/csv/daytime_prj-00_3CSRTT_cond-fixed-iti_mouse-name_day00-phase2-session00___cc_data.h5"
        ...
        if len(start_time_list) == 0:
            ...
            # 'time'列をUNIX秒に変換（combine_date_and_time_to_unixを使って）
            df['time_unix'] = df['time'].apply(lambda t: combine_date_and_time_to_unix(date, t)).astype('float64')
            # actionが'trial_start'の行をフィルタリング
            trial_start_df = df[df['action'] == 'trial_start']
            # time列のみを取得してリストに変換
            start_time_list = trial_start_df['time_unix'].tolist()
        ...
        with h5py.File(hdf5_path, 'r+') as file:
            ...
            for i, trial_num in enumerate(trial_nums):
                ...
                if start_time_path in file:
                    file[start_time_path][()] = start_time_list[i] # 上書き可能
                else:
                ...
                for action in ['Lick', 'NP0', 'NP1', 'NP2', 'NP3', 'NP4']:
                    ...
                    if data_path in file:
                        ...
                        for _, row in action_df.iterrows():
                            time_index = int((row['time_unix'] - trial_start) * downsample_in_hz)  # 20 Hzに基づいてインデックスを計算
            ...
            with h5py.File(hdf5_path, 'r+') as file:
                ...
                for trial_num in df['trial_num'].unique():
                    ...
                    for action in ['OP_NP1_correct', 'OP_NP2_correct', 'OP_NP3_correct']:
                        ...
                        # 各時刻について処理
                        for _, row in action_df.iterrows():
                            time_index = int((row['time_unix'] - trial_start) * operant_downsample_in_hz)  # 20 Hzに基づいてインデックスを計算
                            if 0 <= time_index < len(data_array):
                                data_array[time_index] = 1  # 値を1に更新
