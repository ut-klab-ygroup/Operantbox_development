...
class NosePokeState(State):
    ...
    def _monitor_nose_poke_task(self, phase_settings):
        ...
        handler = functools.partial(self._combined_signal_handler, start_time=start_time, 
                                    ...
                                    lick_time_list=lick_time_list, call_count=call_count)
...
"""
#②sinal handler2つの場合の実装,→signal handlerの性質上動かない。
    def _signal_handler_nose_poke(self, signum, frame, start_time, phase_settings, nose_poke_time_list, 
    ...
    def _signal_handler_lick(self, signum, frame,start_time,phase_settings, lick_time_list):
        current_time = time.perf_counter()
        if current_time - start_time > phase_settings.stimulus_duration_in_s:
    ...
    def _monitor_nose_poke_task(self, phase_settings):
        ...
        while time.perf_counter() - start_time <= phase_settings.stimulus_duration_in_s:
            np_handler = functools.partial(self._signal_handler_nose_poke, start_time=start_time, phase_settings=phase_settings, 
                                        nose_poke_time_list=nose_poke_time_list, 
                                        nose_poke_hole_number_list=nose_poke_hole_number_list,
                                        nose_poke_correct_time_list=nose_poke_correct_time_list, 
                                        nose_poke_hole_number_correct_list=nose_poke_hole_number_correct_list)
            lick_handler = functools.partial(self._signal_handler_lick,start_time,phase_settings, lick_time_list=lick_time_list)

            signal.signal(signal.SIGALRM, np_handler)
            signal.setitimer(signal.ITIMER_REAL, 0.5, 0.5)  # For nose poke
        
            signal.signal(signal.SIGALRM, lick_handler)
            signal.setitimer(signal.ITIMER_REAL, 0.1, 0.1)  # For lick
            #time.sleep(0.1)
...
"""
"""
#thread timerを使った実装：おそらくかなり精度悪い
