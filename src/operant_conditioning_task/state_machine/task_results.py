...
class TaskResults:
    ...
    def __init__(self, results_file_path):
        ...
        self.results_file_writer = csv.writer(self.results_file, delimiter=',')
        self.results_file_writer.writerow(['elapsed_time', 'trial_num', 'state_name', 'action', 'data1', 'data2'])
        ...
        # Lick 行動の時系列データです。
        self.lick = dict()
        self.lick['elapsed_time'] = np.array([], dtype=np.uint64)   # 実験開始からの経過時間 (msec)
        self.lick['trial_num'] = np.array([], dtype=np.uint32)      # 試行番号
        self.lick['state_name'] = np.array([], dtype=np.unicode_)   # 状態の名前
        self.lick['response_time'] = np.array([], dtype=np.uint64)  # キューの呈示から lick 行動までの反応時間 (msec)

        # Nose poke 行動の時系列データです。
        self.nose_poke = dict()
        self.nose_poke['elapsed_time'] = np.array([], dtype=np.uint64)  # 実験開始からの経過時間 (msec)
        self.nose_poke['trial_num'] = np.array([], dtype=np.uint32)     # 試行番号
        self.nose_poke['state_name'] = np.array([], dtype=np.unicode_)  # 状態の名前
        self.nose_poke['target_num'] = np.array([], dtype=np.uint32)    # ターゲットの番号
        self.nose_poke['is_correct'] = np.array([], dtype=np.bool8)     # 正解/不正解
    ...
    def store_lick_results(self, unix_time, trial_num, state_name, response_time):
        ...
        # 結果を保持します。
        elapsed_time = unix_time - self.start_time
        self.lick['elapsed_time'] = np.append(self.lick['elapsed_time'], elapsed_time)
        self.lick['trial_num'] = np.append(self.lick['trial_num'], trial_num)
        self.lick['state_name'] = np.append(self.lick['state_name'], state_name)
        self.lick['response_time'] = np.append(self.lick['response_time'], response_time)

        # 結果ファイルに保存します。
        self.results_file_writer.writerow([elapsed_time, trial_num, state_name,
                                           'Lick', response_time, ''])
    ...
    def store_nose_poke_results(self, unix_time, trial_num, state_name, target_num, is_correct):
        ...
        # 結果を保持します。
        elapsed_time = unix_time - self.start_time
        self.nose_poke['elapsed_time'] = np.append(self.nose_poke['elapsed_time'], elapsed_time)
        self.nose_poke['trial_num'] = np.append(self.nose_poke['trial_num'], trial_num)
        self.nose_poke['state_name'] = np.append(self.nose_poke['state_name'], state_name)
        self.nose_poke['target_num'] = np.append(self.nose_poke['target_num'], target_num)
        self.nose_poke['is_correct'] = np.append(self.nose_poke['is_correct'], is_correct)

        # 結果ファイルに保存します。
        self.results_file_writer.writerow([elapsed_time, trial_num, state_name,
                                           'Nose poke', target_num, is_correct])
    ...
    def check_lick_results(self, num_trials_before, response_time_threshold, rate_threshold):
        total_count = 0
        true_count = 0
        for response_time in reversed(self.lick['response_time']):
    ...
    def check_num_nose_pokes(self, time_before, num_nose_pokes_threshold):
        ...
        # 開始時間と終了時間を求めます。
        max_time = time.time() - self.start_time
        ...
        index_range = self._get_index_range_from_time_range(self.nose_poke['elapsed_time'], min_time, max_time)
    ...
    def check_nose_poke_results(self, time_before):
        ...
        for i in index_range:
            if self.nose_poke['is_correct'][i]:
                correct_count += 1
            total_count += 1
    ...
    def check_total_trials(self, total_trial_threshold):
        return self.total_trials > total_trial_threshold
...
if __name__ == "__main__":
    ...
    # Lick 行動のダミー結果を記録します。
    for i in range(100):
        task_results.store_lick_results(i, 'LickState', i)
    ...
    if check_num_nose_pokes:
        ...
        for i in range(100):
            task_results.store_nose_poke_results(i + 100, 'NosePokeState', i % 2, i % 2 == 0)
        task_results.start_time = time.time()
        assert task_results.check_num_nose_pokes(200, 50) is False
        task_results.start_time = time.time() - 200
    ...
    # check_nose_poke_results() メソッドをいくつかのパターンでテストします。
        ...
        # 条件(B) を試します。
            ...
            for i in range(90):
                task_results.store_nose_poke_results(i + 100, 'NosePokeState', i % 3, i % 3 == 0)
            task_results.start_time = time.time() - 200
            assert task_results.check_nose_poke_results(200) is False
        # 条件(B) を試します。
