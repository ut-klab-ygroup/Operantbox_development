...
#define RESET_POSITION_DELTA 20000    // X steps pull position 
#define ACCEL 60000
#define SPEED 5000
...
void setup() {
  pinMode (3, INPUT_PULLUP); // Input triggering for water, 0V water out, 5V stop
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (incomingByte == 'p') {
      ...
      stepper.moveTo( j ); // set new target position
      while (stepper.currentPosition() < j ){
        stepper.run();
        if(stepper.currentPosition() %2000==0){
                  Serial.println(".");
        }
        if(Serial.read()=='s'){
          break;
